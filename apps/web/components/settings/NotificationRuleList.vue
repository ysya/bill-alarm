    ...
    >
      ...
      >
        <CardContent class="p-4">
          <div class="flex items-center justify-between">
            <div class="space-y-2">
              <div class="h-4 w-40 bg-muted rounded" /><div class="h-3 w-56 bg-muted rounded" />
            </div>
    ...
    <Card v-else-if="rules.length === 0">
      <CardContent class="flex flex-col items-center justify-center py-12 text-center">
        ...
        <Button
          ...
          @click="emit('create')"
        >
          <Plus class="mr-2 h-4 w-4" />建立第一條規則
        </Button>
      </CardContent>
    </Card>
          ...
          <div class="flex items-center justify-between gap-4">
            <div class="min-w-0 flex-1 space-y-1.5">
              ...
              <p class="text-xs text-muted-foreground">
                到期前 {{ rule.daysBefore }} 天，於 {{ rule.timeOfDay }} 發送通知
              </p>
            </div>
            <div class="flex items-center gap-2 shrink-0">
              <Switch
                :model-value="rule.isActive"
                @update:model-value="handleToggleActive(rule)"
              />
              <Button
                variant="ghost"
                size="icon"
              ...
              >
                <Pencil class="h-4 w-4" /><span class="sr-only">編輯</span>
              </Button>
              <Button
                variant="ghost"
                size="icon"
              ...
              >
                <Trash2 class="h-4 w-4" /><span class="sr-only">刪除</span>
              </Button>
            </div>
          </div>
        </CardContent>
