...
- **member 中央 allow-list（在 authGuard 內，deny by default）**，比對 method + path：
  - `GET /api/bills`、`GET /api/bills/summary`、`GET /api/bills/:id`、`GET /api/bills/:id/pdf`、`PATCH /api/bills/:id/pay`、`POST /api/bills/:id/unpay`（新端點：把已繳改回待繳，member 標錯可自行復原；兩種角色的「恢復為待繳」都改走它，避免為此開放整個 PATCH 編輯面）
...
- **member 介面收斂**：
  ...
  - 帳單頁：member 僅「標記已繳／恢復為待繳」，隱藏編輯／刪除／重解析
